...
function ChainCodeValue({
  ...
  crewManifest,
  alignment,
}: Readonly<{ render: boolean; chainCodeDecoder: ChainCodeDecoder, crewManifest: CrewManifest, alignment: string }>) {
  if (render) {
    ...
    if (alignment !== chainCodeDecoder.chainCodeAlignment()) {
      let originalAgent = crewManifest.getLeaders().filter(agent => agent.alignment === alignment)[0];
      originalAlignment = <Card>
