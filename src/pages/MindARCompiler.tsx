...
const MindARCompiler = () => {
    ...
    const startCompilation = async () => {
        ...
        try {
            ...
            const dataList = await compiler.compileImageTargets(imageElements, (p: number) => {
                setProgress(p);
    ...
    return (
        <div style={{ padding: '20px', color: 'white', maxWidth: '800px', margin: '0 auto' }}>
            ...
            {!isReady && <div style={{ color: 'orange' }}>Loading MindAR library...</div>}
            ...
            <div style={{ background: '#222', padding: '20px', borderRadius: '8px', marginTop: '20px' }}>
                ...
                {progress > 0 && progress < 100 && (
                    <div style={{ width: '100%', height: '10px', background: '#444', marginTop: '10px', borderRadius: '5px' }}>
                        <div style={{ width: `${progress}%`, height: '100%', background: '#00ff00', borderRadius: '5px', transition: 'width 0.3s' }}></div>
                    </div>
                )}
            </div>

            {downloadUrl && (
                <div style={{ background: '#1a1a1a', padding: '20px', borderRadius: '8px', marginTop: '20px', border: '1px solid #444' }}>
                    <h3>3. Download</h3>
                    <a
                        href={downloadUrl}
